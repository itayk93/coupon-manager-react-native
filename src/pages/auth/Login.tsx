...
export default function Login() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    setIsLoading(true);
    try {
      const legacyUser = await signInLegacy(data.email, data.password);
      setLegacySession(legacyUser);
      toast.success("התחברת בהצלחה!");
      navigate(from, { replace: true });
    } catch (error: any) {
      toast.error(error.message || "שגיאה בהתחברות. אנא בדוק את הפרטים ונסה שוב.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="login-container">
        ...
        <section className="auth-card">
          ...
          <div className="auth-links">
            ...
            <span>|</span>
            <Link to="/register">אין לך חשבון? הרשמה</Link>
          </div>
