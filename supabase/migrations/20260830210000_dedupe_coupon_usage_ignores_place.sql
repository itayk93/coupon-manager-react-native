-- Duplicate detection stopped keying on the place name.
--
-- The previous rule (20260830143000) required amount + place + minute to all
-- match. OCR broke it: the same Arcaffe branch came back as "ארקפה - מידטאון"
-- from one screenshot and "ארקפה - מיזטאון" from the next, so an identical
-- 15₪ usage at 2026-08-23 09:19 was inserted twice on coupon 843. The place is
-- the noisiest field in the chain and the least identifying — a coupon cannot
-- be spent at two places in the same minute for the same amount.
--
-- New rule, per coupon: amount (to agorot) + timestamp (to the minute).
--
-- Rows with no explicit used_at are exempt. They are stamped with now(), so
-- several such rows in one import share a minute and would otherwise collapse
-- into one. This mirrors `findExistingUsageMatch` in src/lib/usageDuplicateMatch.ts.
...
begin
  ...
  select
    ...
    -- Only a row that carries its own timestamp can be judged a duplicate.
    nullif(x.used_at, '') is not null as has_used_at
  ...
  where t.has_used_at
    and earlier.has_used_at
    and earlier.source_row < t.source_row
