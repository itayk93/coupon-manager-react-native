...
class ShareViewController: UIViewController {
  ...
  private func showReady() {
    ...
    titleLabel.text = "התמונה נשמרה"
    detailLabel.text = "פותחים את Coupon Master — הזיהוי כבר רץ ומסך האישור יעלה בעוד רגע."
    fillProgress(to: 1)
    celebrateMascot()
  ...
  private func buildInterface() {
    ...
    titleLabel.text = "קורא את הקופון"
    titleLabel.font = .systemFont(ofSize: 20, weight: .bold)
    titleLabel.textColor = Brand.ink
    titleLabel.textAlignment = .center
    titleLabel.numberOfLines = 0

    detailLabel.text = "שומרים את הצילום ומכינים אותו לזיהוי…"
